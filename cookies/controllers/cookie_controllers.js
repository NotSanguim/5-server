...
router.get("/cookie-jar", (req, res) => {
    ...
    check?.cart?.forEach(obk => {
        // Pushing new values to the cartUnupdated array
        cartUnupdated.push({...obj});
        // Pushing items from db that match the names of items in our cart, and if they havea  lower price found in the db
        offer.push(
            ...db.filter((i) => {
    ...
    const total = check?.cart?.reduce((acc, i) => (acc += i.price).toFixed(2), 0 );
